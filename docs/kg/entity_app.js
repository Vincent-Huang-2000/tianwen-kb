...
// Derive source book from ref parameter
...
if(ent.contexts&&ent.contexts.length>0){
  ...
  // Separate: current book vs others
  ...
  // Take up to 5 adjacent other-book contexts
  var shownOthers=otherCtxs.slice(0,5);
  
  // Build context display
  h+='<div class="section"><h2>原文语境 <span style="font-size:0.75rem;color:var(--text-muted);font-weight:normal">（共'+ctxs.length+'条'+(srcBook?'，点击跳转到出处':'')+'）</span></h2>';
  ...
  if(srcBook&&curCtxs.length>0){
    ...
    curCtxs.forEach(function(c){
      var chUrl=URLS[c.book]||"";
      var findUrl=chUrl+"#find="+encodeURIComponent(name);
      h+='<div class="ctx-item current-book" onclick="window.open(\''+findUrl+'\')" style="cursor:pointer" title="点击跳转到原文"><span class="ctx-seq">#'+c.seq+'</span>'+esc(c.text)+' <small>('+esc(c.book)+')</small></div>';
    });
  }
  ...
  if(shownOthers.length>0){
    ...
    shownOthers.forEach(function(c){
      var chUrl=URLS[c.book]||"";
      var findUrl=chUrl+"#find="+encodeURIComponent(name);
      h+='<div class="ctx-item" onclick="window.open(\''+findUrl+'\')" style="cursor:pointer" title="点击跳转到原文"><span class="ctx-seq">#'+c.seq+'</span>'+esc(c.text)+' <small>('+esc(c.book)+')</small></div>';
    });
    if(otherCtxs.length>5){
      h+='<div class="ctx-more" style="text-align:center;color:var(--text-muted);font-size:0.78rem;padding:8px">… 还有 '+(otherCtxs.length-5)+' 条来自其他书籍的上下文</div>';
    }
  }
...
if(rm){
  ...
  if(bk){
    bk.href="../chapters/"+ref+"#find="+encodeURIComponent(name);
    bk.textContent="\u2190 返回 "+ref.replace(".html","");
  }
}
})();
